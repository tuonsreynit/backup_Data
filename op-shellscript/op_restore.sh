...
BACKUP_DIR="$HOME2/op_backups"            # Local directory to store downloaded backups temporarily
RESTORE_DIR="$HOME2/op_restore"          # Directory to extract and restore files
REMOTE_NAME="gdrive"                 # Rclone remote name
REMOTE_PATH="OpenProject_Backup"              # Google Drive folder name
...
BACKUP_LIST=$(rclone --config "$RCLONE_CONF" lsf "${REMOTE_NAME}:${REMOTE_PATH}/" | grep "backup_op_.*\.zip")
